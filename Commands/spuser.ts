...
const spuser = async (interaction: ChatInputCommandInteraction, client: Client): Promise<boolean> => {
    ...
    if (interaction.options.getSubcommand() === 'set') {
        ...
        await interaction.editReply({content: "Successfully added the user " + userObj.username + " with ID `" + userObj.id + "` to " + "'" + perms +"' perms."})
        ...
        if ("individualAdminPerms" in configObj) {
            for (let i = 0; i < configObj.individualAdminPerms.length; i++) {
                if (configObj.individualAdminPerms[i] === userObj.id) {
                    ...
                    await collections.config.updateOne({}, {$set: {individualAdminPerms: configObj.individualAdminPerms}})
                    removed = true
                    break
                } 
            }
        ...
        if ("individualUserPerms" in configObj && !removed) {
            for (let i = 0; i < configObj.individualUserPerms.length; i++) {
                if (configObj.individualUserPerms[i] === userObj.id) {
                    ...
                    await collections.config.updateOne({}, {$set: {individualUserPerms: configObj.individualUserPerms}})
                    removed = true
                    break
                } 
            }
        }
        if (removed) await interaction.editReply({content: "Successfully removed `" + userObj.username + "`'s permissions."})
        else await interaction.editReply({content: "`" + userObj.username + "` does not have any permissions in Stockpiler Bot."})
    }
