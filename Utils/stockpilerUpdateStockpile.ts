...
const stockpilerUpdateStockpile = async (client: Client, body: any, response: http.ServerResponse) => {
    ...
    if (await argon2.verify(password.password, body.password)) {
        ...
        if (body.name === "") {
            ...
            const [stockpileHeader, stockpileMsgs, targetMsg, stockpileMsgsHeader] = await generateStockpileMsg(true, "GUILD_ID")
            await updateStockpileMsg(client, "GUILD_ID", [stockpileHeader, stockpileMsgs, targetMsg, stockpileMsgsHeader])
